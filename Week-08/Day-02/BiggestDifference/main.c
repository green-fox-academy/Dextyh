...
int main() {
    ...
    for (int i = 0; i < number_of_classes; ++i) {
        ...
        printf("What is the name of the %d. class?\n", counter);
        {
            char name[50];
            scanf("%s", name);
            array[i].name = malloc((1 + strlen(name)) * sizeof(char));
...
void biggest_difference(class_t *array, int number_of_classes) {
    int result_max = 0;
    int result_min = 101;
    char* class_name = array[0].name;

    int* difference = malloc(number_of_classes * sizeof(int));
